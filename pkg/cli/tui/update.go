...
// handleKeyPress processes all non-shift key presses.
func (m *topModel) handleKeyPress(msg tea.KeyPressMsg) (tea.Model, tea.Cmd) {
	...
	// viewModeTable key handling
	switch {
	case key.Matches(msg, m.keys.Quit):
		return m, tea.Quit
	// Group action key bindings (shift modifier)
	case key.Matches(msg, m.keys.GroupStop):
		m.prepareGroupStopConfirm()
		return m, nil
	case key.Matches(msg, m.keys.GroupRestart):
		m.prepareGroupRestartConfirm()
		return m, nil
	case key.Matches(msg, m.keys.GroupRemove):
		m.prepareGroupRemoveConfirm()
		return m, nil
	case key.Matches(msg, m.keys.GroupToggle):
		...
		if m.groupHighlightNamespace != nil {
			...
			ns := namespaceOfSelected(m)
			if ns != "-" {
				m.groupHighlightNamespace = &ns
			}
		}
		return m, nil
...
// handleMouse processes mouse messages.
...
// handleLogMsg processes log messages from the tail command.
func (m *topModel) handleLogMsg(msg logMsg) {
